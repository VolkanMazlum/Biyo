...
public class EvoAlgo {
  ...
  private static String mutate(String parent, double rate){    //yeni cumleyi aliyoruz
    String retVal = "";
    for(int i = 0;i < parent.length(); i++){
      retVal += (rand.nextDouble() <= rate) ?
        possibilities[rand.nextInt(possibilities.length)]: //eger uyuyorsa harfi rastgele aliriz
        parent.charAt(i);  // eger oran uymuyorsa harfi degistirmeden aliriz
    }
    return retVal;
  }
